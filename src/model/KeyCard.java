...
public class KeyCard {
    ...
    public KeyCard(String keyCardInit) {
        ...
        switch (cardColor) {
            case 'R':
                whoGoesFirst = CardType.RED;
            case 'B':
                whoGoesFirst = CardType.BLUE;
        }
        // iterate through keyCardColors setting the key
        for (int i = 0; i < 25; i++) {
            switch (keyCardColors.charAt(i)) {
                case 'R':
                    this.key[i] = CardType.RED;
                    break;
                case 'B':
                    this.key[i] = CardType.BLUE;
                    break;
                case 'A':
                    this.key[i] = CardType.ASSASSIN;
                    break;
                case 'Y':
                    this.key[i] = CardType.BYSTANDER;
                    break;
            }
        }
